...
# -*- coding: utf-8 -*-
"""
طلایار v11.2 FINAL — دستیار هوشمند بازار طلا
نسخهٔ فروش حرفه‌ای:
  ✅ BRS API واقعی (طلا، دلار، ارز، کریپتو)
  ✅ انس جهانی (XAU/USD)
  ✅ نمودار کندلی حرفه‌ای همه دارایی‌های اصلی + کش بهینه
  ✅ Trial VIP ۳ روزه
  ✅ VIP + پرداخت + پنل ادمین
  ✅ هشدار قیمت (عددی + درصدی + تکرارشونده)
  ✅ گزارش روزانه
  ✅ نمودار کندلی ۲۴ ساعت، ۷ روز و ۳۰ روز برای ارز، طلا، سکه و کریپتو
...
APP_VERSION = "11.2.0"
BUILD_TAG = "FINAL-CHARTS-SUPPORT-COMMANDS"
...
def help_text():
    return (
        ...
        "📊 نمودارهای کندلی حرفه‌ای ۲۴ساعته، ۷روزه و ۳۰روزه\n"
...
def vip_text(user_id):
    ...
    return ("⭐ <b>عضویت VIP طلایار</b>\n\n"
            ...
            "✅ گزارش روزانه خودکار\n"
            "✅ نمودار کندلی حرفه‌ای ۷ و ۳۰ روزه برای دارایی‌های اصلی\n"
...
# ═══════════════════════════════════════════════════════════════
# PROFESSIONAL CANDLE CHARTS (ALL MAIN ASSETS)
# ═══════════════════════════════════════════════════════════════
_CHART_SYMBOLS = {
    "usd": "USD/IRT", "gold18": "GOLD 18K", "emami": "EMAMI COIN",
    "half": "HALF COIN", "quarter": "QUARTER COIN", "ounce": "XAU/USD",
...
def _local_candle_rule(hours):
    if hours <= 24:
        return "15min", "۱۵ دقیقه"
    if hours <= 168:
        return "1h", "۱ ساعت"
    return "4h", "۴ ساعت"
...
def _build_local_ohlc(asset, period_hours):
    ...
    rule, timeframe_label = _local_candle_rule(period_hours)
    ohlc = frame.set_index("dt")["price"].resample(rule).ohlc().dropna()
    if len(ohlc) < 2:
        return None
    coverage_hours = max(0, int((int(frame["ts"].iloc[-1]) - int(frame["ts"].iloc[0])) / 3600))
    return {"df": ohlc, "unit": str(frame["unit"].iloc[-1] or "").strip(),
            "timeframe": timeframe_label, "coverage_hours": coverage_hours,
            "started_ts": int(frame["ts"].iloc[0]), "volume": False,
            "source": "آرشیو واقعی طلایار (نمونه‌برداری ۵ دقیقه‌ای)"}


def _build_ounce_ohlc(period_hours):
    ticker = yf.Ticker("GC=F")
    if period_hours <= 24:
        period, interval, timeframe = "5d", "15m", "۱۵ دقیقه"
    elif period_hours <= 168:
        period, interval, timeframe = "1mo", "1h", "۱ ساعت"
    else:
        period, interval, timeframe = "3mo", "1h", "۱ ساعت"
    frame = ticker.history(period=period, interval=interval)
    if frame.empty:
        return None
    cutoff = frame.index.max() - timedelta(hours=period_hours)
    frame = frame[frame.index >= cutoff].dropna(subset=["Open", "High", "Low", "Close"])
    if frame.empty:
        return None
    cols = ["Open", "High", "Low", "Close"] + (["Volume"] if "Volume" in frame.columns else [])
    frame = frame[cols]
    return {"df": frame, "unit": "USD", "timeframe": timeframe,
            "coverage_hours": period_hours, "started_ts": None,
            "volume": "Volume" in frame.columns, "source": "بازار جهانی"}


def _render_candle_chart(asset, period_hours):
    info = _build_ounce_ohlc(period_hours) if asset == "ounce" else _build_local_ohlc(asset, period_hours)
    if not info:
        return None
    frame = info["df"]
    close_col = "Close" if "Close" in frame.columns else "close"
    first = float(frame[close_col].iloc[0]); last = float(frame[close_col].iloc[-1])
    change = ((last - first) / first) * 100 if first else 0.0
    ema20 = frame[close_col].ewm(span=20, adjust=False).mean()
    addplots = [mpf.make_addplot(ema20, width=1.15, color="#f5c542")] if len(frame) >= 5 else []
    mc = mpf.make_marketcolors(up="#16c784", down="#ea3943", edge="inherit", wick="inherit", volume="inherit")
    style = mpf.make_mpf_style(marketcolors=mc, figcolor="#111827", facecolor="#111827",
        edgecolor="#374151", gridcolor="#263244", gridstyle="--",
        rc={"axes.labelcolor":"#d1d5db","xtick.color":"#9ca3af","ytick.color":"#9ca3af",
            "axes.titlecolor":"#f9fafb","font.size":9})
    buf=BytesIO(); symbol=_CHART_SYMBOLS.get(asset, asset.upper())
    kwargs=dict(type="candle",style=style,figsize=(12,6.75),
        title=f"\nTALAYAR  •  {symbol}  •  {_period_short(period_hours)}",
        ylabel=("$" if asset=="ounce" else (info["unit"] or "PRICE")),
        hlines=dict(hlines=[last],colors=["#6b7280"],linestyle="--",linewidths=[0.8]),
        tight_layout=True, savefig=dict(fname=buf,dpi=145,bbox_inches="tight",facecolor="#111827"))
    if addplots: kwargs["addplot"]=addplots
    if info["volume"]: kwargs["volume"]=True
    mpf.plot(frame,**kwargs)
    image_bytes=buf.getvalue(); buf.close()
    emoji="🟢" if change>=0 else "🔴"
    price_text=f"${last:,.2f}" if asset=="ounce" else f"{_format_number(last)} {info['unit']}".strip()
    coverage_note=""
    if asset!="ounce" and info["coverage_hours"] < int(period_hours*0.8):
        started=datetime.fromtimestamp(info["started_ts"],TEHRAN_TZ).strftime("%Y/%m/%d")
        coverage_note=f"\n⏳ آرشیو از <code>{started}</code> در حال تکمیل است."
    caption=(f"{emoji} <b>نمودار کندلی {ALERT_ASSETS[asset]['label']}</b>\n"
        f"⏳ بازه: {_period_label(period_hours)}\n🕯 تایم‌فریم: {info['timeframe']}\n"
        f"💰 قیمت فعلی: <code>{price_text}</code>\n📈 تغییر بازه: <code>{change:+.2f}%</code>\n"
        f"📊 EMA20: خط طلایی\n🗂 منبع: {info['source']}{coverage_note}")
    return image_bytes, caption


async def send_candle_chart(update_or_query, context, asset, period_hours):
    key=(asset,int(period_hours)); cached=_chart_cache_get(key)
    try:
        if cached: image_bytes,caption=cached
            ...
            async with _chart_render_semaphore:
                ...
                if cached: image_bytes,caption=cached
                    ...
                    rendered=await asyncio.to_thread(_render_candle_chart,asset,int(period_hours))
                    if not rendered: return False
                    image_bytes,caption=rendered; _chart_cache_put(key,image_bytes,caption)
        photo=BytesIO(image_bytes); photo.name=f"talayar_{asset}_{period_hours}.png"
        if isinstance(update_or_query,Update):
            await update_or_query.message.reply_photo(photo=photo,caption=caption,parse_mode="HTML",reply_markup=chart_period_menu(asset,candle=True))
        else:
            await update_or_query.edit_message_text("📊 در حال آماده‌سازی نمودار کندلی...")
            await context.bot.send_photo(chat_id=update_or_query.message.chat.id,photo=photo,caption=caption,parse_mode="HTML",reply_markup=chart_period_menu(asset,candle=True))
        return True
    except Exception:
        logger.exception("Candle chart error for %s/%s",asset,period_hours); return False
...
async def buttons_v2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("chart_asset:"):
        asset = data.split(":", 1)[1]
        await q.edit_message_text("بازه نمودار کندلی را انتخاب کن:", reply_markup=chart_period_menu(asset, candle=True))
        return
    ...
    if data.startswith("chart:"):
        ...
        if not sent:
            await q.edit_message_text("❌ آرشیو این دارایی برای ساخت کندل کافی نیست؛ با جمع‌شدن داده بیشتر خودکار فعال می‌شود.", reply_markup=chart_period_menu(asset, candle=True))
        return
    ...
    if data.startswith("candle:"):
        ...
        if not sent:
            await q.edit_message_text(
                "❌ دادهٔ کندلی این دارایی فعلاً کافی نیست؛ چند دقیقه دیگر دوباره امتحان کنید.",
