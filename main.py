...
# -*- coding: utf-8 -*-
"""
طلایار v15.1.8 Pre-Launch Trading Assistant — دستیار هوشمند بازار طلا، ارز و کریپتو
نسخهٔ فروش حرفه‌ای:
...
APP_VERSION = "15.1.7"
BUILD_TAG = "MINIAPP-HOME-STATE-RECOVERY-BUBBLE-PRESERVED-NEWS-FIX"
...
def refresh_market_news(force=False):
    ...
    try:
        ...
        raw_unique={}
        for n in gathered:
            # اصلاح: normalize title برای جلوگیری از تکرار (مثل "دلار 85,000" vs "نرخ دلار 85 هزار")
            title_normalized = n["title"].lower()
            # حذف علائم و اعداد برای مقایسه بهتر
...
def recent_market_news(limit=30, category="", asset_key=""):
    ...
    with db._conn() as c:
        c.row_factory=sqlite3.Row
        # اصلاح: DISTINCT برای جلوگیری از duplicate (اگر یک news_id دو بار در DB باشد)
        rows=c.execute("""
...
def _mini_price_item(market, key):
    ...
    if bubble:
        ...
        # اضافه شد: پیش‌فرض‌های امن حتی اگر bubble None باشد
    ...
    if user_id:
        ...
        for ck in selected_keys[:crypto_limit(user_id)]:
            ...
            if not spec:
                continue
            t=tickers.get(ck) or {}
            # اصلاح: اضافه کنید bubble data برای cryptocurrency
...
def _mini_asset_payload(market, user_id, asset):
    ...
    # اصلاح: fair_value و bubble_z برای تمام assets (نه فقط gold/crypto)
    if asset in ALERT_ASSETS and asset != "herat_usd":
        bubble = _fair_value_snapshot(market, asset)
        if bubble:
            result["fair_value"] = float(bubble.get("fair", 0))
            result["bubble_z"] = _bubble_z(asset)[0] if bubble else None
        else:
            result["fair_value"] = None
            result["bubble_z"] = None
...
def _mini_save_preferences_payload(user_id, prefs):
    # اصلاح: اضافه کنید success flag تا modal frontend بتواند بسته شود
