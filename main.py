...
APP_VERSION = "13.0.0"
BUILD_TAG = "MINIAPP-MELTED-GOLD-DATA-SAFE"
...
def help_text():
    return (
        ...
        "🎁 دعوت دوستان، اشتراک گزارش/تحلیل و دریافت VIP رایگان\n\n"
...
def _derived_melted_from_gold18(market):
    ...
    if price is None:
        return None
    p = _to_toman(price, (item or {}).get("unit"))
...
def _melted_snapshot(market):
    ...
    if cash is not None:
        cash = _to_toman(cash, (cash_item or {}).get("unit")); cash_source = "بازار"
    else:
        cash = _derived_melted_from_gold18(market); cash_source = "برآورد از طلای ۱۸" if cash is not None else "ناموجود"
    if future is not None:
        future = _to_toman(future, (future_item or {}).get("unit"))
    def tv(item):
        p=_item_price(item); return _to_toman(p, (item or {}).get("unit")) if p is not None else None
...
def _dashboard_payload(market, user_id=None):
    capture_history(market); items=[]
    for key in ("gold18","melted","melted_future","emami","usd","herat_usd","aed","ounce","btc"):
        item=find_alert_item(market,key); price=_item_price(item)
        if price is None and key=="melted": price=_derived_melted_from_gold18(market); unit="تومان"; change=0.0; source="derived"
        elif price is None: continue
        else: unit=(item or {}).get("unit") or ""; change=_safe_change_percent(item); source="market"
        items.append({"key":key,"label":ALERT_ASSETS[key]["label"],"price":price,"unit":unit,"change":change,"source":source})
    return {"version":APP_VERSION,"vip":bool(user_id and is_vip(user_id)),"items":items,"melted":_melted_snapshot(market),"updated_at":datetime.now(TEHRAN_TZ).strftime("%H:%M:%S")}
...
MINIAPP_HTML = base64.b64decode("").decode("utf-8")
...
class PaymentCallbackHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            length=min(int(self.headers.get("Content-Length","0") or 0),65536); payload=json.loads(self.rfile.read(length).decode("utf-8") or "{}"); uid=_verify_telegram_init_data(str(payload.get("initData") or ""))
            if uid is None: code=401; body=json.dumps({"error":"unauthorized"}).encode()
            else:
                market,error=get_market_data()
                if not market: code=503; body=json.dumps({"error":error or "market unavailable"},ensure_ascii=False).encode()
                else: code=200; body=json.dumps(_dashboard_payload(market,uid),ensure_ascii=False).encode()
            ...
            body=b'{"error":"bad request"}'; self.send_response(400); self.send_header("Content-Type","application/json; charset=utf-8"); self.send_header("Content-Length",str(len(body))); self.end_headers(); self.wfile.write(body)
