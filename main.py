...
def _landing_page_html():
    ...
    price30 = html.escape(str(VIP_PRICE_30))
    price90 = html.escape(str(VIP_PRICE_90))
    bot_cta = (
        f"<a class='btn primary' href='{bot_url}' target='_blank' rel='noopener'>ورود به ربات تلگرام</a>"
        if bot_url else
        f"<a class='btn primary' href='{support_url}' target='_blank' rel='noopener'>پشتیبانی و ورود به طلایار</a>"
    )
...
<head>
  ...
  <meta name='description' content='طلایار؛ دستیار هوشمند بازار طلا، ارز و کریپتو با قیمت لحظه‌ای، هشدار، نمودار تکنیکال و ابزارهای حرفه‌ای.'>
  <title>طلایار | دستیار هوشمند بازار طلا</title>
  <style>
    :root{{--bg:#090b10;--panel:#11151d;--panel2:#151b25;--line:#292f3b;--gold:#e5bd51;--gold2:#f4d878;--text:#f7f8fa;--muted:#a9b0bd;--green:#28c67a;--shadow:0 24px 80px #0007}}
    *{{box-sizing:border-box}} html{{scroll-behavior:smooth}} body{{margin:0;background:radial-gradient(circle at 75% 0,#33281755 0,transparent 34%),var(--bg);color:var(--text);font-family:Tahoma,Arial,sans-serif;line-height:1.9}}
    a{{color:inherit}} .wrap{{width:min(1120px,calc(100% - 32px));margin:auto}} .nav{{position:sticky;top:0;z-index:20;background:#090b10dd;backdrop-filter:blur(14px);border-bottom:1px solid #ffffff0f}}
    .navin{{height:72px;display:flex;align-items:center;justify-content:space-between;gap:16px}} .brand{{display:flex;align-items:center;gap:11px;text-decoration:none;font-weight:900;font-size:22px}}
    .mark{{width:42px;height:42px;border:1px solid #8a6b28;border-radius:14px;display:grid;place-items:center;background:linear-gradient(145deg,#272015,#0e1015);color:var(--gold);font-size:22px;box-shadow:inset 0 0 20px #e5bd5112}}
    .navlinks{{display:flex;gap:20px;align-items:center;font-size:14px;color:var(--muted)}} .navlinks a{{text-decoration:none}} .navlinks a:hover{{color:var(--gold2)}}
    .hero{{padding:88px 0 52px}} .heroGrid{{display:grid;grid-template-columns:1.15fr .85fr;gap:38px;align-items:center}} .badge{{display:inline-flex;gap:8px;align-items:center;border:1px solid #705820;background:#e5bd510d;color:var(--gold2);border-radius:999px;padding:7px 12px;font-size:13px}}
    h1{{font-size:clamp(38px,7vw,70px);line-height:1.25;margin:18px 0 14px;letter-spacing:-1px}} .gold{{color:var(--gold2)}} .lead{{color:#c9ced7;font-size:clamp(16px,2vw,20px);max-width:720px;margin:0 0 26px}}
    .actions{{display:flex;gap:12px;flex-wrap:wrap}} .btn{{display:inline-flex;align-items:center;justify-content:center;min-height:50px;padding:0 20px;border-radius:14px;text-decoration:none;font-weight:800;border:1px solid var(--line);transition:.2s}}
    .btn.primary{{background:linear-gradient(135deg,var(--gold2),#bd8f26);color:#14110a;border-color:#c49730;box-shadow:0 10px 34px #d3a53822}} .btn.secondary{{background:#151922;color:#fff}} .btn:hover{{transform:translateY(-2px)}}
    .trust{{display:flex;gap:16px;flex-wrap:wrap;margin-top:24px;color:var(--muted);font-size:13px}} .trust span::before{{content:'✓';color:var(--green);margin-left:6px;font-weight:900}}
    .phone{{background:linear-gradient(160deg,#171c25,#0c0f15);border:1px solid #343a45;border-radius:32px;padding:18px;box-shadow:var(--shadow);position:relative;overflow:hidden}} .phone::before{{content:'';position:absolute;width:220px;height:220px;background:#e5bd5117;filter:blur(45px);border-radius:50%;top:-100px;left:-70px}}
    .screen{{position:relative;border:1px solid #2e3541;background:#0b0e14;border-radius:24px;padding:20px}} .screenTop{{display:flex;justify-content:space-between;align-items:center;margin-bottom:22px}} .live{{color:var(--green);font-size:12px}} .priceCard{{background:linear-gradient(145deg,#191f29,#11151c);border:1px solid #303744;border-radius:18px;padding:16px;margin:10px 0}}
    .row{{display:flex;justify-content:space-between;gap:12px;align-items:center}} .value{{font-size:21px;font-weight:900}} .mini{{font-size:12px;color:var(--muted)}} .up{{color:var(--green)}}
    section{{padding:64px 0}} .sectionHead{{max-width:700px;margin-bottom:28px}} .sectionHead h2{{font-size:clamp(28px,4vw,42px);margin:0 0 8px}} .sectionHead p{{color:var(--muted);margin:0}}
    .grid{{display:grid;grid-template-columns:repeat(3,1fr);gap:14px}} .card{{background:linear-gradient(145deg,#121720,#0e1218);border:1px solid var(--line);border-radius:20px;padding:22px}} .icon{{font-size:28px}} .card h3{{margin:8px 0 4px;font-size:18px}} .card p{{color:var(--muted);font-size:14px;margin:0}}
    .plans{{display:grid;grid-template-columns:repeat(2,1fr);gap:16px;max-width:840px}} .plan{{position:relative;background:#11161e;border:1px solid #333a46;border-radius:22px;padding:26px}} .plan.featured{{border-color:#8a6b28;box-shadow:inset 0 0 50px #e5bd5109}} .plan .tag{{position:absolute;top:14px;left:14px;color:#17120a;background:var(--gold2);border-radius:999px;padding:4px 9px;font-size:11px;font-weight:900}}
    .plan h3{{font-size:24px;margin:0}} .money{{font-size:34px;font-weight:900;color:var(--gold2);margin:8px 0}} .money small{{font-size:13px;color:var(--muted);font-weight:400}} .list{{list-style:none;padding:0;margin:16px 0 0;color:#d6d9df;font-size:14px}} .list li{{margin:7px 0}} .list li::before{{content:'✓';color:var(--green);font-weight:900;margin-left:8px}}
    .security{{display:grid;grid-template-columns:1fr 1fr;gap:18px;align-items:center;background:linear-gradient(135deg,#171c25,#10141b);border:1px solid #363d48;border-radius:24px;padding:28px}} .shield{{font-size:72px;text-align:center;filter:drop-shadow(0 10px 22px #0008)}}
    .notice{{border-right:3px solid var(--gold);background:#e5bd5109;padding:12px 14px;border-radius:10px;color:#d7dae0;font-size:13px;margin-top:18px}} footer{{border-top:1px solid #ffffff12;padding:30px 0 42px;color:var(--muted);font-size:13px}} .foot{{display:flex;justify-content:space-between;gap:20px;flex-wrap:wrap}} .foot a{{text-decoration:none;color:#d8dbe1;margin-left:14px}}
    @media(max-width:860px){{.heroGrid{{grid-template-columns:1fr}} .phone{{max-width:620px}} .grid{{grid-template-columns:1fr 1fr}} .security{{grid-template-columns:1fr}} .shield{{display:none}} .navlinks a:not(.navCta){{display:none}}}}
    @media(max-width:560px){{.wrap{{width:min(100% - 22px,1120px)}} .hero{{padding-top:56px}} .grid,.plans{{grid-template-columns:1fr}} .navin{{height:64px}} .brand{{font-size:19px}} .actions .btn{{width:100%}}}}
  </style>
</head>
<body>
  <nav class='nav'><div class='wrap navin'>
    <a class='brand' href='/'><span class='mark'>ط</span><span>طلایار</span></a>
    <div class='navlinks'><a href='#features'>امکانات</a><a href='#vip'>VIP</a><a href='#security'>امنیت</a><a href='/terms'>قوانین</a><a class='navCta' href='{support_url}' target='_blank' rel='noopener'>پشتیبانی</a></div>
  </div></nav>

  <main>
    <header class='hero'><div class='wrap heroGrid'>
      <div>
        <span class='badge'>● دستیار هوشمند بازار ایران</span>
        <h1>بازار را سریع‌تر ببین؛<br><span class='gold'>هوشمندتر تصمیم بگیر.</span></h1>
        <p class='lead'>طلایار ابزار یکپارچه رصد طلا، ارز و کریپتو است؛ از قیمت لحظه‌ای و هشدار تا نمودار تکنیکال، سبد شخصی و تحلیل الگوریتمی.</p>
        <div class='actions'>{bot_cta}<a class='btn secondary' href='#features'>دیدن امکانات</a></div>
        <div class='trust'><span>قیمت‌های لحظه‌ای</span><span>هشدار هوشمند</span><span>۳ روز VIP آزمایشی</span></div>
      </div>
      <div class='phone' aria-label='نمایش نمونه داشبورد طلایار'><div class='screen'>
        <div class='screenTop'><b style='color:var(--gold2)'>طلایار</b><span class='live'>● بازار آنلاین</span></div>
        <div class='priceCard'><div class='row'><span>طلای ۱۸ عیار</span><span class='mini'>لحظه‌ای</span></div><div class='row'><span class='value'>داده زنده بازار</span><span class='up'>هشدار ✓</span></div></div>
        <div class='priceCard'><div class='row'><span>دلار آمریکا</span><span class='mini'>رصد تغییرات</span></div><div class='row'><span class='value'>نمودار + تحلیل</span><span class='up'>فعال</span></div></div>
        <div class='priceCard'><div class='row'><span>مرکز نوسان VIP</span><span class='mini'>RSI / EMA</span></div><div class='row'><span class='value' style='color:var(--gold2)'>Opportunity Scanner</span><span class='mini'>24H · 7D · 30D</span></div></div>
      </div></div>
    </div></header>

    <section id='features'><div class='wrap'>
      <div class='sectionHead'><h2>همه ابزارهای بازار، در یک دستیار</h2><p>طلایار برای رصد روزانه بازار طراحی شده؛ نه برای شلوغ‌کردن صفحه با اطلاعات بی‌استفاده.</p></div>
      <div class='grid'>
        <article class='card'><div class='icon'>📊</div><h3>قیمت لحظه‌ای</h3><p>طلا، سکه، دلار، انس جهانی و ارزهای دیجیتال با داده واقعی بازار.</p></article>
        <article class='card'><div class='icon'>🔔</div><h3>هشدار قیمت</h3><p>هشدار عددی، درصدی و تکرارشونده برای دارایی‌های مهم بازار.</p></article>
        <article class='card'><div class='icon'>📈</div><h3>نمودار تکنیکال</h3><p>کندل، EMA20/EMA50، RSI14 و سطوح حمایت و مقاومت در چند بازه زمانی.</p></article>
        <article class='card'><div class='icon'>🤖</div><h3>تحلیل هوشمند</h3><p>تحلیل الگوریتمی مبتنی بر RSI، EMA، MACD و ساختار حرکت قیمت.</p></article>
        <article class='card'><div class='icon'>💼</div><h3>سبد و بازار من</h3><p>ثبت دارایی‌ها، رصد عملکرد و Watchlist شخصی برای دسترسی سریع.</p></article>
        <article class='card'><div class='icon'>🧮</div><h3>ماشین‌حساب طلا</h3><p>محاسبه کاربردی قیمت طلا با وزن، اجرت، سود و مالیات.</p></article>
      </div>
    </div></section>

    <section id='vip'><div class='wrap'>
      <div class='sectionHead'><h2>عضویت VIP طلایار</h2><p>برای کاربرانی که هشدار، نمودار و تحلیل حرفه‌ای بیشتری نیاز دارند.</p></div>
      <div class='plans'>
        <article class='plan featured'><span class='tag'>محبوب</span><h3>VIP یک‌ماهه</h3><div class='money'>{price30} <small>تومان / ۳۰ روز</small></div><ul class='list'><li>هشدارهای حرفه‌ای و نامحدود</li><li>تحلیل و نمودارهای پیشرفته</li><li>مرکز نوسان و Watchlist VIP</li><li>گزارش روزانه و ابزارهای اختصاصی</li></ul></article>
        <article class='plan'><h3>VIP سه‌ماهه</h3><div class='money'>{price90} <small>تومان / ۹۰ روز</small></div><ul class='list'><li>تمام امکانات VIP یک‌ماهه</li><li>۹۰ روز دسترسی پیوسته</li><li>تمدید و مدیریت اشتراک از ربات</li><li>پشتیبانی مستقیم</li></ul></article>
      </div>
      <div class='notice'>قیمت‌ها و تحلیل‌های طلایار صرفاً ابزار اطلاع‌رسانی و تحلیلی هستند و توصیه قطعی خرید یا فروش محسوب نمی‌شوند.</div>
    </div></section>

    <section id='security'><div class='wrap'><div class='security'>
      <div><h2 style='margin-top:0'>پرداخت و دسترسی امن</h2><p style='color:var(--muted)'>فرآیند خرید VIP به‌صورت سیستمی ثبت می‌شود و فعال‌سازی اشتراک تنها پس از تأیید نهایی تراکنش انجام می‌شود. اطلاعات محرمانه کارت بانکی در طلایار ذخیره نمی‌شود.</p><div class='actions'><a class='btn secondary' href='/privacy'>حریم خصوصی</a><a class='btn secondary' href='/terms'>شرایط استفاده</a></div></div>
      <div class='shield'>🛡️</div>
    </div></div></section>

    <section id='contact'><div class='wrap'><div class='sectionHead'><h2>پشتیبانی طلایار</h2><p>برای پرسش‌های مربوط به اشتراک، پرداخت یا استفاده از ربات، از پشتیبانی تلگرام استفاده کنید.</p></div><div class='actions'><a class='btn primary' href='{support_url}' target='_blank' rel='noopener'>تماس با پشتیبانی</a></div></div></section>
  </main>

  <footer><div class='wrap foot'><div>© طلایار — نسخه {html.escape(APP_VERSION)}</div><div><a href='/privacy'>حریم خصوصی</a><a href='/terms'>شرایط استفاده</a><a href='{support_url}' target='_blank' rel='noopener'>پشتیبانی</a></div></div></footer>
</body></html>"""
...
<meta name='viewport' content='width=device-width,initial-scale=1'><meta name='theme-color' content='#090b10'>
...
class PaymentCallbackHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        mini_routes={"/api/dashboard","/api/overview","/api/asset","/api/scanner","/api/fair-value","/api/candles","/api/preferences","/api/preferences/save","/api/smart-alert","/api/rsi-map","/api/backtest","/api/activity"}
