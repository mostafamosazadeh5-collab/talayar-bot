...
APP_VERSION = "14.0.3"
BUILD_TAG = "AUDIT-PATCHED-RELIABILITY"
...
def _mini_overview_payload(market, user_id=None):
    ...
    items = [x for x in (_mini_price_item(market, k) for k in keys) if x and k!="herat_usd"]
    selected_crypto=[]
    if user_id:
        selected_keys=crypto_watchlist(user_id)
        if selected_keys:
            tickers,_,_=_crypto_tickers()
            for ck in selected_keys[:crypto_limit(user_id)]:
                t=tickers.get(ck) or {}
                if t.get("price") is not None:
                    selected_crypto.append({"key":ck,"label":SUPPORTED_CRYPTO[ck]["label"],"symbol":SUPPORTED_CRYPTO[ck]["symbol"],"price":t.get("price"),"change":t.get("change"),"unit":"USDT","source":t.get("source")})
...
def _crypto_asset_payload(user_id, asset_key):
    ...
    vip=is_vip(user_id); snap=crypto_signal_snapshot(asset_key)
    if not vip:
...
def _signals_payload(user_id):
    ...
    for key in keys:
        snap=crypto_signal_snapshot(key)
...
class PaymentCallbackHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        mini_routes={"/api/dashboard","/api/overview","/api/asset","/api/scanner","/api/fair-value","/api/candles","/api/preferences","/api/preferences/save","/api/smart-alert","/api/rsi-map","/api/backtest","/api/activity","/api/crypto/catalog","/api/crypto/watchlist/save","/api/crypto/asset","/api/crypto/candles","/api/signals","/api/signal-now","/api/glossary"}
        ...
        try:
            ...
            if uid is None:
                ...
                market_routes={"/api/dashboard","/api/overview","/api/asset","/api/scanner","/api/fair-value","/api/rsi-map"}
                market=None; market_error=None
                if route in market_routes:
                    market,market_error=get_market_data()

                if route in {"/api/dashboard","/api/overview"}:
                    # Dashboard must stay usable even if the domestic feed is temporarily unavailable.
                    # Independent sections (crypto, preferences, signals, glossary) can still function.
                    data=_mini_overview_payload(market or {},uid)
                    if market is None:
                        data["market_status"]="degraded"
                        data["market_error"]=market_error or "market unavailable"
                    else:
                        data["market_status"]="ok"
                    code=200
                elif route in {"/api/asset","/api/scanner","/api/fair-value","/api/rsi-map"} and market is None:
                    code=503; data={"error":"market_unavailable","message":market_error or "داده بازار داخلی موقتاً در دسترس نیست"}
                    ...
                    if asset not in SUPPORTED_CRYPTO: data,code={"error":"invalid_asset"},400
                        ...
                        data,code={"version":APP_VERSION,"result":result,"selected":crypto_watchlist(uid),"limit":crypto_limit(uid)},200
                    ...
                    # /api/candles uses Talayar's stored local history and does not require a live BRS request.
...
async def signal_track_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for asset,sigs in by_asset.items():
            ...
            rows,source,truncated=_crypto_klines_since(asset,int(earliest.timestamp()*1000))
            if not rows:
...
def run():
    ...
    app.job_queue.run_repeating(signal_scan_job, interval=SIGNAL_SCAN_INTERVAL, first=75, name="signal-scan")
