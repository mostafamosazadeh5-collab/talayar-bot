...
# -*- coding: utf-8 -*-
"""
طلایار v15.4.9 — دستیار هوشمند بازار طلا، ارز و کریپتو
تغییرات نسبت به v15.4.0:
  ✅ رفع باگ مسیر /api/signals: دریافت مستقیم market cache در همان branch تا Opportunity دیگر با market تعریف‌نشده خالی نشود
  ✅ اصلاح بارگذاری اخبار: اعلام وضعیت refreshing و retry خودکار Mini App تا آماده‌شدن cache خبر
  ✅ اصلاح کامل Opportunity: اتصال /api/signals به موتور واقعی فرصت‌های چنددارایی بر پایه _market_features
  ✅ حذف fallback قدیمی crypto_signal_snapshot_cached از Opportunity که باعث نمایش محدود/تک‌دارایی می‌شد
  ✅ نمایش اقدام BUY/SELL/WAIT، اعتماد تحلیلی، ریسک، دلیل و وضعیت بازار برای فرصت‌های برتر
  ✅ نمایش سطوح Entry / SL / TP برای داده‌های معتبر VIP در بخش‌های پشتیبانی‌شده
  ✅ Auto-Refresh واقعی Opportunity در Mini App هر ۶۰ ثانیه با جلوگیری از درخواست همزمان
  ✅ Retry خودکار اخبار در Mini App تا ۶ تلاش با فاصله ۱ ثانیه هنگام آماده‌نبودن cache
...
APP_VERSION = "15.4.9"
BUILD_TAG = "V15.4.9-OPPORTUNITY-NEWS-AUTOREFRESH"
...
def refresh_market_news(force=False):
    ...
    try:
        gathered=[]
        # Fetch independent feeds concurrently so one slow provider does not
        # delay the complete news refresh.
        from concurrent.futures import ThreadPoolExecutor, as_completed
        with ThreadPoolExecutor(max_workers=min(4,len(NEWS_RSS_QUERIES)), thread_name_prefix="news-feed") as pool:
            futures={pool.submit(_fetch_news_feed,query,category):category for _,query,category in NEWS_RSS_QUERIES}
            for fut in as_completed(futures):
                category=futures[fut]
                try: gathered.extend(fut.result())
    ...
    with db._conn() as c:
        ...
        if category:
            rows=c.execute("""
                ...
                LIMIT ?
            """, (cutoff,category,max(1,min(100,int(limit))))).fetchall()
        else:
            rows=c.execute("""
                SELECT DISTINCT news_id, * FROM market_news
                WHERE published_at>=?
...
def _mini_news_payload(user_id, category=""):
    ...
    refreshing=False
    if not items:
        # Keep the first open responsive; refill news cache in background.
        refreshing=True
...
def _opportunity_items(market, user_id):
    """Structured multi-asset opportunity feed.

    Domestic assets use the local historical series + BRS current market. Crypto
    assets use the already-computed real Binance multi-timeframe signal cache so
    the Opportunity page does not depend on domestic price-history rows for BTC/ETH.
    No synthetic price, probability, or trade level is created here.
    """
    vip=is_vip(user_id)
    rows=[]
    for asset in ALERT_ASSETS:
        # Crypto opportunity: reuse the real signal engine cache populated by signal_scan_job.
        if asset in SUPPORTED_CRYPTO:
            try:
                snap=crypto_signal_snapshot_cached(asset,600)
            except Exception:
                snap=None
            if snap and snap.get("ok") and snap.get("price") is not None:
                score=int(snap.get("score") or 0)
                side=str(snap.get("side") or "NO_SIGNAL")
                action="BUY" if side=="BUY" and score>=SIGNAL_MIN_SCORE else ("SELL" if side=="SELL" and score>=SIGNAL_MIN_SCORE else "WAIT")
                direction="LONG" if action=="BUY" else ("SHORT" if action=="SELL" else "NEUTRAL")
                reason=" • ".join((snap.get("reasons") or [])[:3]) or "تحلیل چندتایم‌فریم"
                row={"asset":asset,"key":asset,"label":ALERT_ASSETS[asset]["label"],
                     "price":snap.get("price"),"unit":"USDT","score":score,
                     "confidence":score,"quality":int(snap.get("data_quality") or 0),
                     "action":action,"direction":direction,"reason":reason,
                     "risk":str(snap.get("ai_risk_note") or "متوسط")[:80],
                     "regime":snap.get("status") or "کریپتو",
                     "v60":None,"v24h":snap.get("change_24h"),
                     "entry_low":snap.get("entry_low") if vip else None,
                     "entry_high":snap.get("entry_high") if vip else None,
                     "stop_loss":snap.get("stop_loss") if vip else None,
                     "tp1":snap.get("tp1") if vip else None,
                     "tp2":snap.get("tp2") if vip else None,
                     "tp3":snap.get("tp3") if vip else None}
                rows.append(row)
                continue
        # Domestic assets: deterministic historical market-feature engine.
        try:
            f=_market_features(asset, market)
        except Exception:
            continue
        score=f.get("score")
        quality=int(f.get("quality") or 0)
        if f.get("price") is None or score is None or quality < 35:
            continue
        score=int(score)
        if score >= 78:
        ...
        reasons=[]
        if f.get("breakout") == "up": reasons.append("شکست رو به بالا")
        elif f.get("breakout") == "down": reasons.append("شکست رو به پایین")
        if f.get("ema9") is not None and f.get("ema21") is not None:
            reasons.append("EMA 9/21 صعودی" if f["ema9"] > f["ema21"] else "EMA 9/21 نزولی")
        if f.get("rsi") is not None: reasons.append(f"RSI {float(f['rsi']):.0f}")
        if f.get("v60") is not None: reasons.append(f"مومنتوم ۱ساعته {float(f['v60']):+.2f}%")
        if f.get("abnormal"): reasons.append("افزایش غیرعادی نوسان")
        reason=" • ".join(reasons[:3]) or f.get("regime") or "همگرایی تکنیکال"
        risk="بالا" if f.get("vol_ratio") and f.get("vol_ratio") >= 2.5 else ("متوسط" if f.get("vol") and f.get("vol") >= 0.05 else "پایین")
        rows.append({"asset":asset,"key":asset,"label":ALERT_ASSETS[asset]["label"],"price":f.get("price"),"unit":f.get("unit"),
                     "score":score,"confidence":score,"quality":quality,"action":action,"direction":direction,"reason":reason,
                     "risk":risk,"regime":f.get("regime"),"v60":f.get("v60"),"v24h":f.get("v24h"),
                     "entry_low":None,"entry_high":None,"stop_loss":None,"tp1":None,"tp2":None,"tp3":None})
    rows.sort(key=lambda x:(x.get("action")=="WAIT",-int(x.get("score") or 0),-int(x.get("quality") or 0)))
    return rows[:7]

def _signals_payload(market, user_id):
    vip=is_vip(user_id)
    try:
        items=_opportunity_items(market,user_id)
        return {"version":APP_VERSION,"vip":vip,"items":items,"performance":signal_performance(30),
                "min_quality":SIGNAL_MIN_DATA_QUALITY,"min_score":SIGNAL_MIN_SCORE,
                "free_note":None if vip else "نمای چنددارایی فرصت‌ها فعال است؛ جزئیات معامله و سطوح ورود فقط برای VIP نمایش داده می‌شود."},200
    except Exception as e:
        logging.exception("Opportunity payload error")
        return {"version":APP_VERSION,"vip":vip,"items":[],"error":str(e)},200
...
def build_opportunity_scanner(market):
    """Opportunity engine v2: multi-asset human readable opportunities."""
    candidates=[]
    for asset in ALERT_ASSETS:
        f=_market_features(asset, market)
        ...
        if f.get("z") is not None and abs(f.get("z") or 0)>2.5: strength -= 8
        candidates.append((strength, asset, f))
    candidates.sort(reverse=True, key=lambda x:x[0])

    lines=["🎯 <b>مرکز فرصت‌های بازار طلایار</b>",
           "تحلیل چند دارایی با داده واقعی بازار؛ این بخش پیشنهاد تحلیلی است و تضمین سود نیست.", ""]
    for rank,(_,asset,f) in enumerate(candidates[:7],1):
        score=int(f.get("score") or 50)
        if score>=65:
            action="🟢 سناریوی خرید / بررسی لانگ"
        elif score<=35:
            action="🔴 سناریوی فروش / بررسی شورت"
        else:
            action="🟡 فعلاً صبر و زیرنظر"
        reasons=[]
        if f.get("breakout") != "none": reasons.append("شکست محدوده")
        if f.get("abnormal"): reasons.append("افزایش نوسان")
        reasons.append(f"روند {f.get('regime','خنثی')}")
        lines.append(f"<b>{rank}. {ALERT_ASSETS[asset]['label']}</b>")
        lines.append(f"قدرت فرصت: {score}/100 | کیفیت داده: {f.get('quality',0)}%")
        lines.append(f"وضعیت: {action}")
        lines.append(f"دلیل: {'، '.join(reasons)}")
        lines.append("")
    if not candidates:
        lines.append("⚪ فعلاً فرصت معتبر پیدا نشد؛ بازار در حالت انتظار است.")
    return "\n".join(lines)
    ...
    def do_POST(self):
        ...
        try:
            ...
            if uid is None:
                ...
                # Initialize the market snapshot in this route. Reusing `market` from
                # another route branch left it undefined and caused the Opportunity
                # page to return a generic error.
                market,age=get_market_data_cached(CACHE_STALE_MAX_SECONDS)
                if age is None or age>CACHE_TTL_SECONDS:
                    request_market_refresh_background(force=True)
                if market is None:
                    data,code={"version":APP_VERSION,"vip":is_vip(uid),"items":[],
                               "error":"market_unavailable",
                               "message":"داده بازار برای محاسبه فرصت‌ها موقتاً در دسترس نیست؛ بروزرسانی در پس‌زمینه ادامه دارد."},503
                else:
                    data,code=_signals_payload(market,uid)
                    if isinstance(data,dict): data.setdefault("market_age",age)
