...
APP_VERSION = "12.0.0"
BUILD_TAG = "NAVASAN-INTELLIGENCE-FINAL"
...
def main_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("⚡ مرکز نوسان VIP", callback_data="navasan")],
...
def smart_alert_rule_menu(asset):
    ...
    if asset in COIN_SPECS or asset == "gold18":
...
def capture_bubble_history(market):
    ...
    if now - last < HISTORY_SAVE_INTERVAL:
        return
    for asset in ("gold18", "emami", "half", "quarter"):
...
def _market_features(asset, market):
    ...
    if vol_ratio is not None and vol_ratio >= 2.5:
        ...
        regime = "رنج/انتقالی"
    snap = _fair_value_snapshot(market, asset)
    bz, bz_n = _bubble_z(asset) if asset in COIN_SPECS or asset == "gold18" else (None, 0)
    return {
...
def build_heatmap(market):
    rows=[]
    for asset in ("usd","gold18","emami","half","quarter","ounce","btc","eth","usdt"):
        f=_market_features(asset, market)
