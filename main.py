...
# Static Telegram keyboards are cached to reduce rebuild overhead on every click.
...
def main_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🎁 دعوت دوستان و دریافت VIP", callback_data="referrals")],
...
def _ensure_json_files():
    ...
    defaults = (
        ...
        (BOT_STATE_FILE, {}), (REFERRALS_FILE, {"referrals": {}, "rewards": {}}),
    )
...
def _referral_store():
    data = _load_json(REFERRALS_FILE, {"referrals": {}, "rewards": {}})
...
def register_referral(new_user_id, referrer_id):
    """Register only a genuinely new user and never allow self-referrals."""
    if str(new_user_id) == str(referrer_id):
        return False
    users = _load_json(USERS_FILE, {})
    if str(referrer_id) not in users:
        return False
    data = _referral_store()
    if str(new_user_id) in data["referrals"]:
        return False
    data["referrals"][str(new_user_id)] = {
    ...
    _save_json(REFERRALS_FILE, data)
    return True


def qualified_referral_count(referrer_id):
    data = _referral_store()
    return sum(
        1 for item in data["referrals"].values()
        if str(item.get("referrer_id")) == str(referrer_id) and item.get("qualified")
    )
...
async def qualify_referral(user_id, context):
    data = _referral_store()
    referral = data["referrals"].get(str(user_id))
    if not referral or referral.get("qualified"):
        return False
    referral["qualified"] = True
    referral["qualified_at"] = _utc_now()
    referrer_id = referral.get("referrer_id")
    count = sum(
        1 for item in data["referrals"].values()
        if str(item.get("referrer_id")) == str(referrer_id) and item.get("qualified")
    )
    reward = data["rewards"].setdefault(str(referrer_id), {"claimed_tiers": []})
    claimed = {int(item) for item in reward.get("claimed_tiers", [])}
    new_rewards = []
    for needed, days in ((3, 7), (10, 30), (25, 90)):
        if count >= needed and needed not in claimed:
            add_vip_user(referrer_id, days, source=f"referral:{needed}")
            claimed.add(needed)
            new_rewards.append((needed, days))
    reward["claimed_tiers"] = sorted(claimed)
    _save_json(REFERRALS_FILE, data)
    try:
        message = f"✅ یک دعوت شما معتبر شد.\nتعداد دعوت‌های معتبر: {count} نفر"
        if new_rewards:
            message += "\n\n" + "\n".join(
                f"🎉 جایزه {needed} دعوت: {days} روز VIP فعال شد."
                for needed, days in new_rewards
            )
...
def add_trial_vip(user_id):
    """Activate one-time 3 day VIP trial for new users."""
    data = _load_json(VIP_USERS_FILE, {})
...
def make_backup():
    ...
    for path in (ALERTS_FILE, VIP_USERS_FILE, DAILY_SUBS_FILE, USERS_FILE,
                 PURCHASES_FILE, HISTORY_FILE, BOT_STATE_FILE, REFERRALS_FILE):
...
async def start_v2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    clear_flow(context)
    users = _load_json(USERS_FILE, {})
    is_new_user = str(update.effective_user.id) not in users
    referral_registered = False
    if is_new_user and context.args:
        match = re.fullmatch(r"ref_(\d+)", context.args[0])
        if match:
            referral_registered = register_referral(update.effective_user.id, int(match.group(1)))
    register_user(update.effective_user, update.effective_chat.id)
    trial_activated = add_trial_vip(update.effective_user.id) if is_new_user else False
...
async def receive_text_v2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if flow == "alert_value":
        ...
        create_alert(alert)
        await qualify_referral(update.effective_user.id, context)
        clear_flow(context)
        await update.message.reply_text(
    ...
    if flow == "calc_fee":
        ...
        raw = weight * price
        fee = raw * value / 100
        seller_profit = (raw + fee) * 0.07
        tax = fee * 0.10
        total = raw + fee + seller_profit + tax
        clear_flow(context)
        await update.message.reply_text(
            f"🧮 <b>محاسبه تقریبی طلا</b>\n\nوزن: {_format_number(weight)} گرم\n"
            f"ارزش خام: {_format_number(raw)} تومان\nاجرت: {_format_number(fee)} تومان\n"
            f"سود فروشنده تقریبی: {_format_number(seller_profit)} تومان\n"
            f"مالیات تقریبی: {_format_number(tax)} تومان\n\n"
            f"قیمت نهایی تقریبی: <b>{_format_number(total)} تومان</b>\n\n"
            "<i>محاسبه تقریبی است و با فاکتور نهایی طلافروش ممکن است متفاوت باشد.</i>", reply_markup=main_menu(), parse_mode="HTML")
        return
...
async def buttons_v2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "referrals_top":
        ...
        if not leaders:
            ...
            for rank, (leader_id, count) in enumerate(leaders, start=1):
                user = users.get(str(leader_id), {})
                name = html.escape(user.get("first_name") or "کاربر طلایار")
    ...
    if data.startswith("admin:"):
        ...
        if action == "stats":
            ...
            referral_data = _referral_store()
            qualified_refs = sum(bool(item.get("qualified")) for item in referral_data["referrals"].values())
            text = (f"📊 آمار\n\nکاربران: {len(users)}\nVIP فعال: {sum(is_vip(i) for i in registry)}\n"
                    f"هشدارها: {len(load_alerts())}\nگزارش فعال: {sum(bool(s.get('active')) for s in subs.values())}\n"
                    f"دعوت معتبر: {qualified_refs}\nخرید منتظر: {sum(p.get('status') == 'pending' for p in purchases)}")
...
def build_ai_summary(market):
    ...
    return "🤖 <b>تحلیل هوشمند طلایار</b>\n\n" + "\n".join(items) + "\n\nاین تحلیل فقط بر اساس تغییرات قیمت است و توصیه خرید یا فروش نیست."
