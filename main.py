...
# -*- coding: utf-8 -*-
"""
طلایار v11.3 Technical Charts — دستیار هوشمند بازار طلا
نسخهٔ فروش حرفه‌ای:
...
APP_VERSION = "11.3.0"
BUILD_TAG = "TECHNICAL-CHARTS-BUTTON-FIX"
...
def _dynamic_rule(coverage_minutes, minimum_minutes=5):
    ...
    for minutes, rule, label in valid:
        if coverage_minutes / minutes <= 80:
...
def _render_candle_chart(asset, period_hours):
    ...
    indicator_line = f"📐 EMA20: <code>{ema20_text}</code>"
    if len(frame) >= 25:
        indicator_line += f" | EMA50: <code>{ema50_text}</code>"
    else:
        indicator_line += " | EMA50: در حال تکمیل"
    rsi_line = f"📊 RSI14: <code>{rsi_last:.1f}</code>" if has_rsi else "📊 RSI14: داده کافی نیست"
