...
# -*- coding: utf-8 -*-
"""
طلایار v15.4.0 — دستیار هوشمند بازار طلا، ارز و کریپتو
تغییرات نسبت به v15.3.1:
  ✅ رفع باگ اصلی «کلیک روی سربرگ اخبار می‌رود به اقتصاد»: تابع go() در جاوااسکریپت
     تا این نسخه با هر بار فراخوانی روی view فعلی «toggle» می‌کرد؛ یعنی نه‌فقط اخبار،
     بلکه هر تعویض دارایی/کریپتو/زیرصفحهٔ «بیشتر» هم همین مشکل را داشت. حالا go() فقط
     در صورت تغییر واقعی view چیزی جابه‌جا می‌کند و دیگر هرگز به‌صورت ناخواسته navigate
     نمی‌کند؛ رفتار toggle (باز/بسته‌شدن با کلیک دوم) فقط برای آیکون ⚙️ در تابع
     toggleSettings() جداگانه پیاده‌سازی شده است.
  ✅ نمایش مبلغ حباب (تومان، بدون درصد) روی صفحه جزئیات هر دارایی — قبلاً این مقدار
     در پاسخ API موجود بود ولی هرگز در صفحه دارایی رندر نمی‌شد.
تغییرات v15.3.1 (حفظ شده): رفع ترتیب except در سرور HTTP مینی‌اپ.
تغییرات v15.3.0 (حفظ شده): رفع کرش صفحه کریپتو، بستن خودکار تنظیمات، محافظ صفحه سفید،
بازگشت به تب خانه، رفع تکرار اخبار، نمایش حباب در لیست قیمت‌های خانه.
تمام قابلیت‌های نسخه‌های قبلی (BRS API، انس جهانی، VIP، هشدار قیمت، گزارش روزانه،
تحلیل تکنیکال، ریفرال، پنل ادمین، Runtime License Lock و غیره) بدون تغییر حفظ شده‌اند.
"""
...
APP_VERSION = "15.4.0"
BUILD_TAG = "V15.4.1-OPPORTUNITY-GUIDE-FIX"
...
class Database:
    ...
    def get_miniapp_preferences(self, user_id):
        defaults = {
            ...
            "pinned": ["gold18", "melted", "usd", "ounce"],
...
def refresh_market_news(force=False):
    ...
    try:
        ...
        raw_unique={}
        for n in gathered:
            title_normalized = n["title"].lower()
            title_normalized = re.sub(r"[^\u0600-\u06FFa-z\s]", "", title_normalized)
            title_normalized = re.sub(r"\s+", " ", title_normalized).strip()
            source_normalized = (n.get("source", "") or "").lower().strip()
            key=hashlib.sha256((title_normalized+"|"+source_normalized).encode("utf-8")).hexdigest()[:24]
...
def recent_market_news(limit=30, category="", asset_key=""):
    ...
    with db._conn() as c:
        c.row_factory=sqlite3.Row
        rows=c.execute("""
            SELECT DISTINCT news_id, * FROM market_news
            WHERE published_at>=?
            GROUP BY news_id
            ORDER BY impact DESC,published_at DESC
            LIMIT ?
        """, (cutoff,max(1,min(100,int(limit))))).fetchall()
    out=[]
    seen_ids=set()
    for r in rows:
        if r["news_id"] in seen_ids: continue
...
def _mini_news_payload(user_id, category=""):
    ...
    if not items:
        # v15.1.7: keep first open responsive; refill news cache in background.
...
def _signal_side_fa(side):
    return {"BUY":"🟢 سناریوی ورود فعال","SELL":"🔴 سناریوی خروج/فروش فعال","NEUTRAL":"⏳ فعلاً منتظر تایید بمان","NO_SIGNAL":"⚠️ داده کافی برای تصمیم وجود ندارد"}.get(str(side or ""),"بازار زیر نظر است")
...
def _signal_state_message(snapshot):
    ...
    return "⏳ فعلاً ورود عجولانه مناسب نیست؛ منتظر تایید شرایط بازار بمان."
...
🆕 <b>مینی‌اپ و ربات v15.4.0</b>
• مرکز دارایی‌های پویا و نمایش دارایی‌های انتخابی کاربر
...
def _bubble_line(snapshot):
    ...
    if not snapshot:
        return ""
    pct=snapshot.get("bubble_pct", snapshot.get("pct"))
    amount=snapshot.get("bubble_amount")
    if amount is None and snapshot.get("bubble") is not None:
        amount=snapshot.get("bubble")
    if amount is None:
        return ""
    try:
...
def _mini_price_item(market, key):
    ...
    if bubble:
        result["bubble_amount"]=float(bubble.get("bubble", 0))
        result["bubble_pct"]=float(bubble.get("pct", 0))
        result["fair_value"]=float(bubble.get("fair", 0))
        result["bubble_kind"]=str(bubble.get("kind", "unknown"))
        result["bubble_reference"]=str(bubble.get("reference", ""))
    else:
        result["bubble_amount"]=None
        result["bubble_pct"]=None
        result["fair_value"]=None
        result["bubble_kind"]="unknown"
        result["bubble_reference"]=""
    return result
    ...
    if user_id:
        ...
        for ck in selected_keys[:crypto_limit(user_id)]:
            ...
            if not spec:
                continue
            t=tickers.get(ck) or {}
            bubble_data = _fair_value_snapshot(market, ck)
            crypto_item = {
            ...
            if bubble_data:
                crypto_item["bubble_pct"] = float(bubble_data.get("pct", 0))
                crypto_item["bubble_amount"] = float(bubble_data.get("bubble", 0))
                crypto_item["fair_value"] = float(bubble_data.get("fair", 0))
            selected_crypto.append(crypto_item)
...
def _mini_asset_payload(market, user_id, asset):
    ...
    if "fair_value" not in result or result.get("fair_value") is None:
        bubble = _fair_value_snapshot(market, asset)
        if bubble:
            result["fair_value"] = float(bubble.get("fair", 0))
            result["bubble_z"] = _bubble_z(asset)[0] if callable(globals().get("_bubble_z")) else None
    return result, 200
...
def _mini_preferences_payload(user_id):
    prefs = db.get_miniapp_preferences(user_id)
    return {"version":APP_VERSION,"preferences":prefs,"success":True,"ok":True}, 200

def _mini_save_preferences_payload(user_id, prefs):
    saved_prefs = db.set_miniapp_preferences(user_id, prefs if isinstance(prefs,dict) else {})
    return {"version":APP_VERSION,"preferences":saved_prefs,"success":True,"closed":True}, 200
...
def _signals_payload(user_id):
    ...
    """
    vip=is_vip(user_id)
    # Opportunity dashboard must not be locked to the first crypto asset.
    # Free users see a compact multi-asset view; VIP users see the full watchlist.
    selected=crypto_watchlist(user_id)
    if vip:
        keys=selected[:crypto_limit(user_id)] if selected else list(DEFAULT_CRYPTO_KEYS[:crypto_limit(user_id)])
    else:
        keys=selected[:3] if selected else list(DEFAULT_CRYPTO_KEYS[:3])
    rows=[]
    for key in keys:
    ...
    return {"version":APP_VERSION,"vip":vip,"items":rows,"performance":signal_performance(30),"min_quality":SIGNAL_MIN_DATA_QUALITY,"min_score":SIGNAL_MIN_SCORE,"free_note":None if vip else "نسخه رایگان نمای چند دارایی را نشان می‌دهد؛ جزئیات ورود، اهداف، حد ضرر و تحلیل پیشرفته مخصوص VIP است."},200
...
MINIAPP_HTML = base64.b64decode("").decode("utf-8")
...
def build_opportunity_scanner(market):
    ...
    lines=["🎯 <b>فرصت‌های بازار طلایار</b>", "بررسی چند بازار بر اساس روند، مومنتوم، کیفیت داده و ریسک. خروجی فقط پیشنهاد تحلیلی است و تضمین سود نیست.", ""]
    for rank,(_,asset,f) in enumerate(candidates[:7],1):
        ...
        score=int(f.get("score") or 50)
        action="🟢 بررسی خرید پله‌ای" if score>=65 else ("🔴 احتیاط / کاهش ریسک" if score<=35 else "🟡 فعلاً صبر و زیرنظر")
        reason="، ".join(flags) if flags else f"روند {f.get('regime','خنثی')}"
        lines.append(f"{rank}. {direction} <b>{ALERT_ASSETS[asset]['label']}</b> — قدرت {score}/100 | Q {f['quality']}%")
        lines.append(f"   اقدام: {action}")
        lines.append(f"   دلیل: {reason} | 1h {_fmt_pct(f['v60'])}")
    if not candidates:
        lines.append("⚪ داده کافی برای فرصت تاییدشده وجود ندارد؛ بازار زیر نظر است و در صورت شکل‌گیری شرایط بهتر اطلاع‌رسانی می‌شود.")
    return "\n".join(lines)
        ...
        try:
            ...
            if uid is None:
                ...
                try:
                    ...
                    logger.exception("Mini App overview render failed")
                    data={
                        "version": APP_VERSION,
                        "items": [],
                        "movers": [],
                        "error": "dashboard_recovery",
                        "message": "بارگذاری داشبورد موقتاً با مشکل روبرو شد؛ دوباره تلاش کنید."
                    }
                if market is None:
            ...
            if heavy_acquired:
                _mini_heavy_slots.release()
                heavy_acquired=False
            logger.exception("Mini App request failed: %s", route)
            body=json.dumps({
                "version": APP_VERSION,
                "error":"internal_error",
                "message":"خطای موقت در بارگذاری اطلاعات. لطفاً دوباره تلاش کنید."
            ...
            try:
                ...
                logger.info("Mini App client disconnected while sending error response for %s", route)
