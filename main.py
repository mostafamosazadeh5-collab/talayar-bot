...
# -*- coding: utf-8 -*-
"""
طلایار v15.4.6 — دستیار هوشمند بازار طلا، ارز و کریپتو
...
APP_VERSION = "15.4.6"
BUILD_TAG = "V15.4.6-OPPORTUNITY-MULTI-ASSET"
...
def _opportunity_items(market, user_id):
    """Structured multi-asset opportunity feed for the Mini App.
    Uses the same _market_features engine as the Telegram opportunity scanner.
    Scores are analytical confidence scores, not guaranteed win probabilities.
    """
    vip=is_vip(user_id)
    rows=[]
    for asset in ALERT_ASSETS:
        ...
        if f.get("rsi") is not None:
            r=float(f["rsi"]); reasons.append(f"RSI {r:.0f}")
        if f.get("v60") is not None:
            reasons.append(f"مومنتوم ۱ساعته {float(f['v60']):+.2f}%")
        ...
        row={"asset":asset,"key":asset,"label":ALERT_ASSETS[asset]["label"],"price":f.get("price"),"unit":f.get("unit"),
             "score":score,"confidence":score,"quality":quality,"action":action,"direction":direction,"reason":reason,
             "risk":risk,"regime":f.get("regime"),"v60":f.get("v60"),"v24h":f.get("v24h"),"entry":None,"stop_loss":None,"targets":None}
        # Only expose trade levels when they come from a real persisted crypto signal.
        if vip and asset in SUPPORTED_CRYPTO:
            try:
                snap=crypto_signal_snapshot_cached(asset,600)
                if snap and snap.get("ok"):
                    row.update({"entry_low":snap.get("entry_low"),"entry_high":snap.get("entry_high"),"stop_loss":snap.get("stop_loss"),
                                "tp1":snap.get("tp1"),"tp2":snap.get("tp2"),"tp3":snap.get("tp3")})
            except Exception:
                pass
        rows.append(row)
    rows.sort(key=lambda x:(x.get("score",0),x.get("quality",0)),reverse=True)
    return rows[:7]
