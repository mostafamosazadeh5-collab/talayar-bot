...
APP_VERSION = "15.1.1"
BUILD_TAG = "RC-LOAD-HARDENING-AI-FAILOVER"
...
def _mini_news_payload(user_id, category=""):
    ...
    return {"version":APP_VERSION,"vip":is_vip(user_id),"items":[{
        "id":x["news_id"],"title":x["title"],"summary":x["summary"],"source":x["source"],"url":x["url"],
        "category":x["category"],"assets":x["assets"],"sentiment":x["sentiment"],"sentiment_label":fa_sent(x["sentiment"]),
        "impact":x["impact"],"reliability":x["reliability"],"published_at":x["published_at"],"ai_used":bool(x["ai_used"])
    } for x in items],"updated_at":_utc_now()}
...
def find_display_item(market,asset_key):
    ...
    if direct:
        d=dict(direct); d.setdefault("_display_source","منبع اصلی بازار"); d["_fallback"]=False; return d
...
def _bubble_line(snapshot):
    if not snapshot:
        return ""
    amount=float(snapshot.get("bubble") or 0)
    pct=float(snapshot.get("pct") or 0)
    icon="🟢" if amount < 0 else "🔴" if amount > 0 else "⚪"
    word="تخفیف" if amount < 0 else "حباب" if amount > 0 else "متعادل"
    return f"\n🫧 {icon} {word}: <b>{abs(pct):.2f}%</b> • <b>{_format_number(abs(amount))}</b> تومان"
