...
# -*- coding: utf-8 -*-
"""
طلایار v13.8 Personal Market Assistant — دستیار هوشمند بازار طلا
نسخهٔ فروش حرفه‌ای:
...
APP_VERSION = "14.0.0"
BUILD_TAG = "WEBSITE-ZARINPAL-READY"
...
VIP_PRICE_30 = os.environ.get("VIP_PRICE_30", "149,000")
VIP_PRICE_90 = os.environ.get("VIP_PRICE_90", "449,000")
...
_market_cache = {"data": None, "saved_at": 0.0, "fetched_at": 0, "last_error": ""}
...
GLOSSARY = {
    "rsi": ("RSI", "شاخص قدرت نسبی؛ مومنتوم قیمت را بین ۰ تا ۱۰۰ می‌سنجد. بالای ۷۰ معمولاً ناحیه داغ و زیر ۳۰ ناحیه ضعیف تلقی می‌شود، اما به‌تنهایی سیگنال خرید یا فروش نیست."),
    "ema": ("EMA", "میانگین متحرک نمایی؛ به قیمت‌های جدید وزن بیشتری می‌دهد. قرار گرفتن EMA کوتاه‌مدت بالای EMA بلندمدت می‌تواند نشانه تقویت روند باشد."),
    "macd": ("MACD", "اندیکاتور مومنتوم و روند بر پایه اختلاف میانگین‌های متحرک. تقاطع MACD و Signal یکی از چند تأیید مورد استفاده طلایار است."),
    "support": ("حمایت", "محدوده‌ای که در گذشته تقاضا یا واکنش خریداران بیشتر شده است. حمایت قطعی نیست و می‌تواند شکسته شود."),
    "resistance": ("مقاومت", "محدوده‌ای که در گذشته عرضه یا فشار فروش بیشتر شده است. شکست معتبر مقاومت نیاز به تأیید دارد."),
    "breakout": ("Breakout / شکست", "عبور قیمت از محدوده مهم همراه با تأیید روند یا حجم. طلایار شکست را به‌تنهایی سیگنال قطعی تلقی نمی‌کند."),
    "volume": ("Volume / حجم", "میزان معامله در یک بازه. افزایش حجم می‌تواند قدرت یک حرکت را تأیید کند."),
    "atr": ("ATR", "میانگین دامنه واقعی؛ برای سنجش نوسان استفاده می‌شود. در v14 برای کریپتو از OHLC واقعی صرافی محاسبه می‌شود."),
    "sl": ("SL / حد ضرر", "سطح ابطال سناریو برای محدودکردن زیان. رسیدن قیمت به SL به معنی نامعتبرشدن ستاپ است."),
    "tp": ("TP / حد سود", "هدف یا اهداف قیمتی سناریو. طلایار می‌تواند TP1، TP2 و TP3 را با نسبت ریسک/بازده ثبت کند."),
    "rr": ("Risk/Reward", "نسبت سود بالقوه به ریسک. مثلاً ۲R یعنی هدف سود دو برابر فاصله ریسک اولیه است."),
    "quality": ("Data Quality", "امتیاز کیفیت داده بر اساس تازگی قیمت، کفایت کندل‌های واقعی و در صورت امکان تطبیق با منبع دوم. زیر آستانه تعیین‌شده سیگنال صادر نمی‌شود."),
    "score": ("Signal Score", "امتیاز همگرایی چند مؤلفه مانند روند، RSI، MACD، شکست، حجم و تأیید چندتایم‌فریمی. این امتیاز تضمین سود نیست."),
}
FAQ_ITEMS = [
    ...
    ("قیمت‌ها و سیگنال‌ها واقعی‌اند؟", "v14 در بخش کریپتو قیمت و OHLCV را از API عمومی بازار دریافت می‌کند و عدد قیمت ساختگی تولید نمی‌کند. اگر داده معتبر یا تازه نباشد، سیگنال صادر نمی‌شود. هیچ سیگنالی تضمین سود ۱۰۰٪ ندارد."),
    ...
    ("سیگنال چگونه ساخته می‌شود؟", "موتور سیگنال از OHLCV واقعی، EMA، RSI، MACD، ATR، حجم، ساختار حمایت/مقاومت و تأیید چند تایم‌فریم استفاده می‌کند. زیر حد کیفیت داده، خروجی NO SIGNAL است."),
]
...
def _crypto_data_quality(asset_key, ticker, rows, cross_price=None):
    ...
    if age>20: q-=25; reasons.append("ticker_stale")
    if len(rows)<120: q-=20; reasons.append("few_candles")
    if rows:
        cage=max(0,(now_ms-int(rows[-1].get("close_time") or now_ms))/1000)
        if cage>7200: q-=25; reasons.append("candle_stale")
    if cross_price and ticker.get("price"):
        dev=abs(float(ticker["price"])-float(cross_price))/float(ticker["price"])*100
        if dev>2.0: q-=35; reasons.append(f"cross_deviation_{dev:.2f}")
        elif dev>1.0: q-=12; reasons.append(f"cross_deviation_{dev:.2f}")
...
def persist_signal(snapshot):
    ...
    with db._conn() as c:
        c.execute("""INSERT INTO market_signals(signal_id,asset_key,symbol,side,timeframe,issued_price,entry_low,entry_high,stop_loss,tp1,tp2,tp3,score,data_quality,source,reasons_json,metrics_json,status,hit_level,created_at,last_checked_at)
                     VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'open',0,?,?)""",
                  (sid,snapshot["asset"],snapshot["symbol"],snapshot["side"],"1H",snapshot["price"],snapshot["entry_low"],snapshot["entry_high"],snapshot["stop_loss"],snapshot["tp1"],snapshot["tp2"],snapshot["tp3"],int(snapshot["score"]),int(snapshot["data_quality"]),snapshot["source"],json.dumps(snapshot.get("reasons") or [],ensure_ascii=False),json.dumps({k:snapshot.get(k) for k in ("rsi","rsi15","ema20","ema50","macd","macd_signal","atr","volume_ratio","support","resistance","cross_price","cross_source")},ensure_ascii=False),_utc_now(),_utc_now()))
        c.execute("INSERT OR IGNORE INTO signal_events(signal_id,event_type,price,event_at,detail) VALUES(?,?,?,?,?)",(sid,"issued",snapshot["price"],_utc_now(),"signal issued from real Binance OHLCV"))
    return sid
...
def signal_performance(days=30):
    ...
    total=len(rows); closed=[r for r in rows if r["status"] in {"stopped","tp3","ambiguous"}]
    wins=sum(1 for r in closed if r["status"]=="tp3" or int(r["hit_level"] or 0)>=1)
    losses=sum(1 for r in closed if r["status"]=="stopped" and int(r["hit_level"] or 0)==0)
    ambiguous=sum(1 for r in closed if r["status"]=="ambiguous")
    resolved=wins+losses
    return {"days":days,"total":total,"wins":wins,"losses":losses,"ambiguous":ambiguous,"open":total-len(closed),"hit_rate":(wins/resolved*100 if resolved else None)}
...
def signal_menu():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("⚡ اسکن همین حالا", callback_data="signal_scan")],
        [InlineKeyboardButton("📊 کارنامه شفاف", callback_data="signal_performance"), InlineKeyboardButton("🧾 آخرین سیگنال‌ها", callback_data="signal_history")],
...
• Gauge عقربه‌ای + Explain Score + نمودار Gap-aware
• RSI Market Map + Backtest Lite + Activity History
• هشدار هوشمند همراه Snapshot نمودار
• Fair Value، بازار من و مرکز آب‌شده Pro
...
• 💼 سبد شخصی + P&L: رایگان ۲ موقعیت، VIP تا ۱۰ موقعیت
• ☀️ Briefing صبحگاهی شخصی بر پایه Watchlist و سبد
• 🕒 نمایش سن داده و کیفیت تازگی قیمت
• 💬 پرسش هوشمند محدود و قابل اتکا بدون تولید عدد ساختگی
• 🔎 توضیح «چرا این هشدار آمد؟»
...
def vip_text(user_id):
    ...
    return ("⭐ <b>عضویت VIP طلایار</b>\n\n"
            ...
            "✅ مرکز نوسان + Signal Engine v14: رادار حباب، اسکنر، هیت‌مپ، Backtest و هشدار هوشمند\n\n"
...
def get_market_data(force_refresh=False):
    ...
    if not force_refresh and _market_cache["data"] is not None and now_mono-_market_cache["saved_at"]<CACHE_TTL_SECONDS:
        return _market_cache["data"],None
    if not BRS_API_URL:
        return None,"تنظیمات اتصال API کامل نیست"
    # Lock prevents many simultaneous Mini App/Bot requests from hammering BRS.
    with _market_refresh_lock:
        now_mono=time.monotonic()
        if _market_cache["data"] is not None and now_mono-_market_cache["saved_at"]<2:
            return _market_cache["data"],None
        try:
            ...
            _market_cache.update({"data":payload,"saved_at":now_mono,"fetched_at":int(time.time()),"last_error":""})
            return payload,None
        except requests.Timeout:
            err="زمان پاسخ‌گویی API تمام شد"
        except requests.RequestException as exc:
            status=exc.response.status_code if exc.response is not None else "no-response"
            err=f"خطای اتصال به API (کد {status})"
        except Exception as exc:
...
def _backtest_lite(asset,rule="confluence",horizon=12):
    ...
    return {"samples":len(events),"hit":wins/len(events)*100,"avg":sum(events)/len(events),"best":max(events),"worst":min(events),
            "note":"Backtest Lite روی تاریخچه محلی طلایار است؛ کارمزد، لغزش و نقدشوندگی را لحاظ نمی‌کند."}
...
def _crypto_asset_payload(user_id, asset_key):
    ...
    snap=crypto_signal_snapshot(asset_key)
    return {"version":APP_VERSION,"vip":is_vip(user_id),"ticker":t,"signal":snap,"selected":asset_key in set(crypto_watchlist(user_id))},200
...
def _signals_payload(user_id):
    ...
    rows=[]
    for key in selected[:crypto_limit(user_id)]:
        snap=crypto_signal_snapshot(key)
        rows.append(snap)
    return {"version":APP_VERSION,"vip":is_vip(user_id),"items":rows,"performance":signal_performance(30),"min_quality":SIGNAL_MIN_DATA_QUALITY,"min_score":SIGNAL_MIN_SCORE},200
...
def _landing_page_html():
    ...
    return f"""<!doctype html><html lang='fa' dir='rtl'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><meta name='theme-color' content='#ffffff'><title>طلایار | دستیار هوشمند بازار</title><meta name='description' content='طلایار؛ رصد واقعی بازار، نمودار، هشدار، سیگنال الگوریتمی و ابزارهای طلا و کریپتو.'><style>
...
</style></head><body><nav><div class='wrap nav'><a class='brand' href='/'>طلایار <i>✦</i></a><div class='links'><a href='#features'>امکانات</a><a href='#signals'>سیگنال</a><a href='#vip'>VIP</a><a href='#learn'>راهنما</a><a href='{support_url}' target='_blank'>پشتیبانی</a></div></div></nav><main><header class='hero'><div class='wrap heroGrid'><div><span class='badge'>دستیار هوشمند بازار طلا، ارز و کریپتو</span><h1>بازار را فقط نبین؛<br><span class='blue'>بفهم.</span></h1><p class='lead'>قیمت واقعی، نمودار حرفه‌ای، هشدار، سبد شخصی و Signal Engine با فیلتر کیفیت داده؛ اگر داده معتبر نباشد، طلایار سیگنال صادر نمی‌کند.</p><div class='actions'>{cta}<a class='btn secondary' href='#signals'>دیدن Signal Engine</a></div></div><div class='mock'><div class='mocktop'><b class='blue'>Talayar Live</b><span class='live'>● Market online</span></div><div class='ticker'><span>BTC/USDT<br><small style='color:var(--muted)'>Real market feed</small></span><b>Live</b></div><div class='ticker'><span>Signal Quality<br><small style='color:var(--muted)'>Freshness + OHLCV + validation</small></span><b class='gold'>Q Gate</b></div><div class='ticker'><span>Verified History<br><small style='color:var(--muted)'>برد و باخت هر دو ثبت می‌شوند</small></span><b>Audit</b></div></div></div></header>
<section id='features'><div class='wrap'><div class='head'><h2>یک داشبورد، چند بازار</h2><p>هسته قدیمی طلایار حفظ شده و قابلیت‌های v14 به‌صورت ماژولار اضافه شده‌اند.</p></div><div class='grid'><div class='card'>⚡<strong>قیمت لحظه‌ای</strong><p>طلا، دلار، انس و کریپتو بدون عدد نمایشی.</p></div><div class='card'>📈<strong>نمودار حرفه‌ای</strong><p>برای کریپتو OHLCV واقعی صرافی؛ RSI، EMA، MACD و ATR.</p></div><div class='card'>🎯<strong>Signal Engine</strong><p>BUY / SELL فقط با امتیاز و کیفیت داده کافی.</p></div><div class='card'>₿<strong>کریپتوهای من</strong><p>۳ ارز برای Free و تا {CRYPTO_VIP_LIMIT} ارز برای VIP از فهرست پشتیبانی‌شده.</p></div><div class='card'>🔔<strong>هشدار و Tracker</strong><p>ثبت سیگنال، TP/SL و تاریخچه قابل حسابرسی.</p></div><div class='card'>📘<strong>راهنمای مفاهیم</strong><p>توضیح ساده اصطلاحات تکنیکال داخل سایت و ربات.</p></div></div></div></section>
<section id='signals'><div class='wrap'><div class='signal'><div><div class='head'><h2>سیگنال واقعی، نه عدد نمایشی</h2><p>نمونه کارت زیر فقط ساختار رابط است؛ اعداد واقعی تنها در لحظه از Market Feed ساخته می‌شوند.</p></div><ul><li>OHLCV واقعی برای کریپتو</li><li>Data Quality Gate</li><li>چند تایم‌فریم 15m / 1h / 4h</li><li>ثبت Entry، SL، TP1/2/3 و ID</li><li>NO SIGNAL هنگام ضعف داده</li></ul></div><div class='sigbox'><span class='buy'>● LIVE SIGNAL FORMAT</span><h3>BTC/USDT</h3><p>Side: BUY / SELL / NO SIGNAL</p><p>Score + Data Quality + Entry + SL + TPs</p><p style='color:var(--muted)'>همه مقادیر از داده واقعی محاسبه می‌شوند؛ این کارت عمداً عدد ساختگی ندارد.</p></div></div></div></section>
...
<section id='learn'><div class='wrap'><div class='head'><h2>راهنمای اصطلاحات</h2><p>برای اینکه کاربر بداند هر کلمه در ربات چه معنی دارد.</p></div><div class='terms'>{glossary}</div></div></section><section><div class='wrap faq'><div class='head'><h2>سوالات متداول</h2></div>{faq}</div></section></main><footer><div class='wrap foot'><div>© طلایار — v{APP_VERSION}</div><div><a href='/privacy'>حریم خصوصی</a> · <a href='/terms'>شرایط استفاده</a> · <a href='{support_url}' target='_blank'>پشتیبانی</a></div></div></footer></body></html>"""
...
async def receive_text_v2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if flow=="portfolio_price":
        ...
        clear_flow(context)
        market,_=get_market_data(force_refresh=True)
...
async def buttons_v2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "signals":
        ...
        await _show_callback_text(q,
            "🎯 <b>Talayar Signal Engine v14</b>\n\n"
            "سیگنال فقط از OHLCV واقعی کریپتو و پس از عبور از فیلتر کیفیت داده صادر می‌شود. "
            "اگر داده کافی یا تازه نباشد، خروجی NO SIGNAL است.\n\n"
    ...
    if data == "crypto_lab":
        ...
        await _show_callback_text(q,f"₿ <b>کریپتوهای من</b>\n\n{len(selected)}/{limit} انتخاب شده. هر ارز انتخابی در Mini App قیمت، OHLCV واقعی، RSI، EMA، MACD، ATR و سیگنال مستقل دارد.",reply_markup=crypto_lab_menu(user_id),parse_mode="HTML"); return
    ...
    if data in {"signal_scan","signal_history","signal_performance"}:
        if data=="signal_performance":
            p=signal_performance(30); hr="—" if p["hit_rate"] is None else f"{p['hit_rate']:.1f}%"
            text=f"📊 <b>کارنامه شفاف سیگنال‌ها — ۳۰ روز</b>\n\nکل: {p['total']}\nبرد ثبت‌شده: {p['wins']}\nSL: {p['losses']}\nباز: {p['open']}\nHit rate بسته‌شده: {hr}\n\n<i>فقط رکوردهای دیتابیس شمرده می‌شوند؛ بردهای انتخابی یا نمایشی حذف/اضافه نمی‌شوند.</i>"
        ...
        if data=="signal_history":
            ...
            for x in sigs: lines.append(f"{('🟢' if x['side']=='BUY' else '🔴')} <b>{html.escape(SUPPORTED_CRYPTO.get(x['asset_key'],{}).get('label',x['asset_key']))}</b> {x['side']} | {x['score']}/100 | Q{x['data_quality']} | {html.escape(x['status'])}")
            if not sigs: lines.append("هنوز سیگنال ثبت‌شده‌ای وجود ندارد.")
            await _show_callback_text(q,"\n".join(lines),reply_markup=signal_menu(),parse_mode="HTML"); return
        keys=crypto_watchlist(user_id) or list(DEFAULT_CRYPTO_KEYS[:crypto_limit(user_id)]); lines=["⚡ <b>اسکن واقعی بازار</b>",""]
        for key in keys[:crypto_limit(user_id)]:
            snap=crypto_signal_snapshot(key,force=True); label=SUPPORTED_CRYPTO[key]["label"]
            if not snap.get("ok"): lines.append(f"⚪ {label}: داده معتبر کافی نیست"); continue
            side=snap.get("side"); icon="🟢" if side=="BUY" else "🔴" if side=="SELL" else "⚪"
            lines.append(f"{icon} <b>{label}</b>: {side} | Score {snap.get('score')} | Q {snap.get('data_quality')} | { _format_number(snap.get('price')) } USDT")
            if side in {"BUY","SELL"}:
                sid=persist_signal(snap)
                lines.append(f"↳ Entry {_format_number(snap['entry_low'])}–{_format_number(snap['entry_high'])} | SL {_format_number(snap['stop_loss'])} | TP1 {_format_number(snap['tp1'])}"+(f" | ID {sid}" if sid else ""))
        lines += ["","<i>قیمت‌ها از داده واقعی بازار گرفته می‌شوند. سیگنال تضمین سود نیست.</i>"]
        await _show_callback_text(q,"\n".join(lines),reply_markup=signal_menu(),parse_mode="HTML"); return
    ...
    if data == "plans":
        await q.edit_message_text(
            ...
            f"کندل حرفه‌ای همه دارایی‌های اصلی، تحلیل هوشمند، بازار من تا {WATCHLIST_VIP_LIMIT} دارایی و مرکز نوسان + Signal Engine v14.",
    ...
    if data in {"portfolio","portfolio_refresh"}:
        market,error=get_market_data(force_refresh=(data=="portfolio_refresh"))
        if not market:
...
async def portfolio_snapshot_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    market,error=get_market_data()
    if not market:
        logger.warning("Portfolio snapshot skipped: %s",error)
        return
...
async def signal_scan_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for key in keys:
            snap=crypto_signal_snapshot(key)
            if not snap.get("ok") or snap.get("side") not in {"BUY","SELL"}: continue
            sid=persist_signal(snap)
            if not sid: continue
            with db._conn() as c:
                users=[int(r[0]) for r in c.execute("SELECT user_id FROM crypto_watchlist WHERE asset_key=?",(key,)).fetchall()]
            for uid in users:
                if not is_vip(uid): continue
                try:
                    side=snap["side"]; icon="🟢" if side=="BUY" else "🔴"
                    reason=" • ".join(snap.get("reasons") or [])
                    await context.bot.send_message(uid,
                        f"{icon} <b>سیگنال واقعی طلایار — {html.escape(SUPPORTED_CRYPTO[key]['label'])}</b>\n\n"
                        f"{side} | Score {snap['score']}/100 | Data Q {snap['data_quality']}/100\n"
                        f"قیمت: <b>{_format_number(snap['price'])} USDT</b>\n"
                        f"Entry: {_format_number(snap['entry_low'])} تا {_format_number(snap['entry_high'])}\n"
                        f"SL: {_format_number(snap['stop_loss'])}\nTP1: {_format_number(snap['tp1'])}\nTP2: {_format_number(snap['tp2'])}\nTP3: {_format_number(snap['tp3'])}\n"
                        f"ID: <code>{sid}</code>\n\n{html.escape(reason)}\n\n<i>منبع: Binance public OHLCV — بدون تضمین سود.</i>",parse_mode="HTML")
                except Exception: logger.exception("Signal notify failed for %s",uid)
    except Exception:
...
async def signal_track_job(context: ContextTypes.DEFAULT_TYPE):
    if not SIGNAL_ENGINE_ENABLED: return
    try:
        open_signals=recent_signals(100)
        open_signals=[x for x in open_signals if x.get("status") in {"open","tp1","tp2"}]
        by_asset={}
        for sig in open_signals: by_asset.setdefault(sig["asset_key"],[]).append(sig)
        for asset,sigs in by_asset.items():
            rows,_=_crypto_klines(asset,"1m",6,force=True)
            if not rows: continue
            high=max(x["h"] for x in rows[-4:]); low=min(x["l"] for x in rows[-4:]); last=rows[-1]["c"]
            for sig in sigs:
                side=sig["side"]; new_status=sig["status"]; hit=int(sig.get("hit_level") or 0); event=None; detail=""
                stop_hit=(low<=sig["stop_loss"] if side=="BUY" else high>=sig["stop_loss"])
                tp1_hit=(high>=sig["tp1"] if side=="BUY" else low<=sig["tp1"]); tp2_hit=(high>=sig["tp2"] if side=="BUY" else low<=sig["tp2"]); tp3_hit=(high>=sig["tp3"] if side=="BUY" else low<=sig["tp3"])
                if stop_hit and (tp1_hit or tp2_hit or tp3_hit):
                    new_status="ambiguous"; event="ambiguous"; detail="SL and TP touched inside the same 1m observation window; result not counted as win."
                elif stop_hit:
                    new_status="stopped"; event="sl"; detail="Stop-loss touched"
                elif tp3_hit:
                    new_status="tp3"; hit=3; event="tp3"; detail="TP3 touched"
                elif tp2_hit and hit<2:
                    new_status="tp2"; hit=2; event="tp2"; detail="TP2 touched"
                elif tp1_hit and hit<1:
                    new_status="tp1"; hit=1; event="tp1"; detail="TP1 touched"
                if event:
                    now=_utc_now(); closed=now if new_status in {"stopped","tp3","ambiguous"} else ""
                    with db._conn() as c:
                        c.execute("UPDATE market_signals SET status=?,hit_level=?,closed_at=CASE WHEN ?<>'' THEN ? ELSE closed_at END,last_checked_at=? WHERE signal_id=?",(new_status,hit,closed,closed,now,sig["signal_id"]))
                        c.execute("INSERT OR IGNORE INTO signal_events(signal_id,event_type,price,event_at,detail) VALUES(?,?,?,?,?)",(sig["signal_id"],event,last,now,detail))
                else:
                    with db._conn() as c: c.execute("UPDATE market_signals SET last_checked_at=? WHERE signal_id=?",(_utc_now(),sig["signal_id"]))
    except Exception:
