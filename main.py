...
# -*- coding: utf-8 -*-
"""
طلایار v15.1.13 MiniApp Stability Patch — دستیار هوشمند بازار طلا، ارز و کریپتو
نسخهٔ فروش حرفه‌ای:
...
APP_VERSION = "15.1.13"
BUILD_TAG = "MINIAPP-NAV-WHITEPAGE-FIX-HOME-FIRSTLOAD-PATCH"
...
def _mini_news_payload(user_id, category=""):
    # v15.1.9: prevent blank first load and remove duplicate news cards.
    items=recent_market_news(40,category=category)
    if not items:
        try:
            refresh_market_news(force=True)
            items=recent_market_news(40,category=category)
        except Exception:
            threading.Thread(target=refresh_market_news,kwargs={"force":True},daemon=True,name="news-refresh-on-demand").start()
            items=recent_market_news(40,category=category)

    # Semantic-lite deduplication: same/similar titles from multiple feeds are merged.
    unique=[]
    seen=[]
    for item in items:
        raw=" ".join(str(item.get("title","")).lower().split())
        if not raw:
            continue
        key=raw[:90]
        duplicate=False
        for old in seen:
            common=len(set(key.split()) & set(old.split()))
            total=max(len(set(key.split())),1)
    ...
    for x in items:
        ...
        out.append({
            ...
            "ai_used":bool(x["ai_used"]),"news_score":max(1,min(100,int((int(x.get("impact",1))*25)+(int(x.get("reliability",60))*0.5)))),"language":"fa","fa_mode":fa_mode
        })
