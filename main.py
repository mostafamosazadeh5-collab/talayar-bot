...
# -*- coding: utf-8 -*-
"""
طلایار v15.3.1 — دستیار هوشمند بازار طلا، ارز و کریپتو
این نسخه نتیجه یک بازبینی کامل و خط‌به‌خط تمام ۹۲۴۰ خط فایل است (نه فقط بخش مینی‌اپ).
تغییرات نسبت به v15.3.0:
  ✅ رفع باگ واقعی در سرور HTTP مینی‌اپ: ترتیب اشتباه except در do_POST باعث می‌شد
     except BrokenPipeError هرگز اجرا نشود (چون except Exception عمومی زودتر همه چیز
     از جمله قطعی اتصال کاربر موبایل را می‌گرفت و سعی می‌کرد روی سوکت بسته بنویسد).
تغییرات v15.3.0 (حفظ شده):
  ✅ رفع بسته نشدن Settings با کلیک دوم، رفع کرش صفحه کریپتو (bubbleLine تعریف‌نشده)
  ✅ بستن خودکار Settings بعد از ذخیره، محافظ صفحه سفید هنگام بوت
  ✅ بازگشت خودکار به تب خانه با هر بار فعال‌شدن مینی‌اپ
  ✅ رفع تکرار اخبار (نرمال‌سازی عنوان + DISTINCT + dedup فرانت‌اند)
  ✅ نمایش مبلغ حباب (بدون درصد) کنار قیمت‌های خانه
  ✅ حباب/ارزش منصفانه برای همه دارایی‌ها، حتی وقتی None است
...
APP_VERSION = "15.3.1"
BUILD_TAG = "FULL-AUDIT-BROKENPIPE-EXCEPT-ORDER-FIX"
