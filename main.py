...
def refresh_market_news(force=False):
    ...
    try:
        ...
        # علاوه بر لینک/منبع، شباهت عنوان خبرهای چند RSS مختلف هم بررسی می‌شود.
        raw_unique={}
        for n in gathered:
            title_norm=re.sub(r"[^a-zA-Z0-9\u0600-\u06FF]+"," ",(n.get("title") or "").lower()).strip()
            key=hashlib.sha256((title_norm+"|"+n.get("source","")).encode("utf-8")).hexdigest()[:24]
            duplicate=False
            for old in raw_unique.values():
                old_title=re.sub(r"[^a-zA-Z0-9\u0600-\u06FF]+"," ",(old.get("title") or "").lower()).strip()
                if title_norm and old_title and difflib.SequenceMatcher(None,title_norm,old_title).ratio() >= 0.82:
                    duplicate=True
                    break
            if not duplicate:
                raw_unique[key]={**n,"news_id":key}
