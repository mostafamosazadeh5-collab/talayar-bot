...
APP_VERSION = "15.5.0"
BUILD_TAG = "V15.5.0-SCALE-NOTIFY-QUEUE-BATCH-VIP"
...
# ═══════════════════════════════════════════════════════════════
# ═══════════════════════════════════════════════════════════════
# SCALABLE TELEGRAM NOTIFICATION QUEUE
# ═══════════════════════════════════════════════════════════════
NOTIFY_QUEUE_MAXSIZE = max(1000, int(os.environ.get("NOTIFY_QUEUE_MAXSIZE", "10000")))
NOTIFY_WORKERS = max(2, min(12, int(os.environ.get("NOTIFY_WORKERS", "6"))))
NOTIFY_RETRIES = max(1, min(5, int(os.environ.get("NOTIFY_RETRIES", "3"))))
NOTIFY_BASE_DELAY = max(0.25, min(5.0, float(os.environ.get("NOTIFY_BASE_DELAY", "0.75"))))

class NotificationQueue:
    """Bounded, deduplicating Telegram sender.

    Producers only enqueue work; Telegram I/O is performed by a small worker pool.
    This prevents large fan-outs from blocking market/signal jobs and also absorbs
    short Telegram 429/network bursts without spawning one task per recipient.
    """

    def __init__(self, maxsize=NOTIFY_QUEUE_MAXSIZE, workers=NOTIFY_WORKERS):
    ...
    def _ensure_started(self):
        ...
        self._started = True
        logger.info("Notification queue started | workers=%s maxsize=%s", self.workers, self.maxsize)

    async def put(self, key, sender, on_success=None, on_failure=None):
        ...
        async with self._lock:
            ...
            self._accepted += 1
        try:
            # Backpressure instead of dropping notifications when the queue is full.
            await self.queue.put((key, sender, on_success, on_failure))
            return True
        except Exception:
            async with self._lock:
    ...
    async def _worker(self, worker_id):
        while True:
            key, sender, on_success, on_failure = await self.queue.get()
            ok = False
            try:
                for attempt in range(NOTIFY_RETRIES):
                    try:
                        ...
                        delay = max(float(getattr(exc, "retry_after", 1.0)), 0.5)
                        logger.warning("Telegram rate limit | worker=%s key=%s sleep=%.1fs", worker_id, key, delay)
                        await asyncio.sleep(delay)
                    except Exception:
                        if attempt + 1 >= NOTIFY_RETRIES:
                            logger.exception("Notification failed | worker=%s key=%s", worker_id, key)
                            break
                        delay = NOTIFY_BASE_DELAY * (2 ** attempt)
                        await asyncio.sleep(delay)
                if ok:
                    self._completed += 1
                    if on_success is not None:
                        try:
                            ...
                            logger.exception("Notification success callback failed | key=%s", key)
                    ...
                    try:
                        ...
                        logger.exception("Notification failure callback failed | key=%s", key)
            finally:
                if not ok:
                    self._failed += 1
                async with self._lock:
...
class Database:
    ...
    def get_vip_statuses(self, user_ids):
        """Return active VIP status for many users with one SQL query."""
        ids = sorted({int(uid) for uid in user_ids if str(uid).lstrip("-").isdigit()})
        if not ids:
            return {}
        ...
        with self._conn() as c:
            rows = c.execute(
                f"""SELECT user_id, expires_at FROM vip_users
                    WHERE user_id IN ({placeholders})
                    AND (expires_at IS NULL OR expires_at = '' OR expires_at > ?)""",
                (*ids, now),
            ).fetchall()
        active = {int(r[0]) for r in rows}
        return {uid: uid in active for uid in ids}
...
def refresh_market_news(force=False):
    ...
    try:
        gathered=[]
        # Independent feeds are fetched concurrently; a slow provider cannot serialize the refresh.
...
def _free_signal_push_allowed(user_id):
    ...
    with _FREE_SIGNAL_PENDING_LOCK:
        if uid in _FREE_SIGNAL_PENDING:
            return False
    day=datetime.now(TEHRAN_TZ).strftime("%Y-%m-%d")
    return db.get_state(f"free_signal_push:{uid}","") != day


def _reserve_free_signal_push(user_id):
    uid=int(user_id)
    if is_vip(uid): return True
    with _FREE_SIGNAL_PENDING_LOCK:
        if uid in _FREE_SIGNAL_PENDING:
            return False
        day=datetime.now(TEHRAN_TZ).strftime("%Y-%m-%d")
        if db.get_state(f"free_signal_push:{uid}","") == day:
            return False
        _FREE_SIGNAL_PENDING.add(uid)
        return True


def _release_free_signal_push(user_id):
    with _FREE_SIGNAL_PENDING_LOCK:
        _FREE_SIGNAL_PENDING.discard(int(user_id))

def _mark_free_signal_push(user_id):
    uid=int(user_id)
    with _FREE_SIGNAL_PENDING_LOCK:
        _FREE_SIGNAL_PENDING.discard(uid)
    if not is_vip(uid):
        db.set_state(f"free_signal_push:{uid}",datetime.now(TEHRAN_TZ).strftime("%Y-%m-%d"))
...
def vip_statuses(user_ids):
    """Batch VIP lookup; admins are always VIP without hitting SQLite."""
    ids={int(uid) for uid in user_ids if str(uid).lstrip("-").isdigit()}
    result=db.get_vip_statuses(ids)
    for uid in ids:
        if _is_admin(uid):
            result[uid]=True
    return result
...
def _signals_payload(user_id):
    """Mini App opportunity dashboard connected to the real opportunity engine."""
    vip=is_vip(user_id)

    # Use the multi-asset opportunity engine instead of the old crypto-only snapshot.
    try:
        market = get_market_snapshot() if 'get_market_snapshot' in globals() else {}
        engine = build_opportunity_scanner(market)

        items = []
        if isinstance(engine, dict):
            raw = engine.get("items", [])
        ...
        for item in raw:
            ...
            public_item = {
                ...
                "strength": item.get("score") or item.get("strength"),
        ...
        if not items:
            # Safe fallback keeps old snapshot behaviour only when engine has no data.
            selected=crypto_watchlist(user_id)
            keys=selected[:crypto_limit(user_id)] if vip else selected[:3]
            for key in keys:
                snap=crypto_signal_snapshot_cached(key,600)
                if snap:
                    items.append(snap if vip else _signal_public_teaser(snap))

        return {
            "version": APP_VERSION,
            "vip": vip,
            "items": items,
            "performance": signal_performance(30),
            "min_quality": SIGNAL_MIN_DATA_QUALITY,
            "min_score": SIGNAL_MIN_SCORE,
            "free_note": None if vip else "نسخه رایگان نمای فرصت‌ها را نشان می‌دهد؛ جزئیات ورود، حد ضرر و اهداف مخصوص VIP است."
        }, 200
...
async def check_smart_alerts(context: ContextTypes.DEFAULT_TYPE, market):
    ...
    if not alerts:
        return
    statuses=vip_statuses([a.get("user_id") for a in alerts])
    feature_cache={}
    for a in alerts:
        uid=int(a.get("user_id") or 0)
        if not statuses.get(uid, False):
            continue
        ...
        image_bytes = rendered[0] if rendered else None
        async def sender(a=a, caption=caption, image_bytes=image_bytes):
            if image_bytes:
                photo=BytesIO(image_bytes); photo.name="talayar_alert.png"
        ...
        async def on_success(a=a, now=now, asset=asset, detail=detail, f=f):
        ...
        await notification_queue.put(
            f"smart-alert:{a['id']}:{now // SMART_ALERT_COOLDOWN_SECONDS}",
            sender,
...
async def portfolio_snapshot_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    if not market or error:
        ...
        return
    user_ids=db.portfolio_user_ids()
    if not user_ids:
        return
    ...
    async def one(uid):
        async with sem:
            try:
                await asyncio.to_thread(save_portfolio_snapshot_for_user,uid,market)
            except Exception:
...
async def daily_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    due = [s for s in subs if s.get("active") and s.get("time") == now.strftime("%H:%M") and s.get("last_sent") != today]
    if not due:
        return
    market, _ = await get_market_data_async()
    if not market:
        return
    bot_username = await get_bot_username(context)
    statuses=vip_statuses([s.get("user_id") for s in due])
    for sub in due:
        uid=int(sub.get("user_id") or 0)
        if not statuses.get(uid, False):
            with db._conn() as c:
                c.execute("UPDATE daily_subs SET active = 0 WHERE user_id = ?", (uid,))
            continue
        try:
            save_portfolio_snapshot_for_user(uid,market)
            report=build_personal_briefing(uid,market)
            markup=share_report_menu(uid, bot_username, market)
            async def on_success(uid=uid, today=today):
                with db._conn() as c:
                    c.execute("UPDATE daily_subs SET last_sent = ? WHERE user_id = ?", (today, uid))
            await notification_queue.put(
                f"daily-report:{uid}:{today}",
                lambda chat_id=int(sub["chat_id"]), report=report, markup=markup: context.bot.send_message(
                    chat_id, report, parse_mode="HTML", reply_markup=markup
                ),
                on_success=on_success,
            )
        except Exception:
            logger.exception("Daily report enqueue failed for %s",uid)
        ...
        if stage is not None:
            ...
            if row.get("last_reminder") != reminder_key:
                ...
                try:
                    ...
                    async def on_success(user_id=user_id, reminder_key=reminder_key, stage=stage):
                        db.update_vip_reminder(user_id, reminder_key)
                        if stage == "expired":
                            with db._conn() as c:
                                c.execute("UPDATE daily_subs SET active = 0 WHERE user_id = ?", (user_id,))
                    await notification_queue.put(
                        f"vip-reminder:{user_id}:{reminder_key}",
                        lambda chat_id=chat_id, text=text, markup=markup: context.bot.send_message(
...
async def _signal_watchers(asset_key, vip_only=True):
    """Get signal recipients without N+1 VIP queries."""
    with db._conn() as c:
        rows=c.execute(
            "SELECT DISTINCT user_id FROM crypto_watchlist WHERE asset_key=?",
            (asset_key,),
        ).fetchall()
    ids=[int(r[0]) for r in rows if r]
    if not vip_only:
        return ids
    statuses=vip_statuses(ids)
    return [uid for uid in ids if statuses.get(uid, False)]
...
async def _notify_signal_event(context, sig, event_type, price, detail=""):
    ...
    recipients = await _signal_watchers(asset, vip_only=True)
    for uid in recipients:
        await notification_queue.put(
            f"signal-event:{sig.get('signal_id')}:{event_type}:{uid}",
            lambda uid=uid, text=text: context.bot.send_message(uid,text,parse_mode="HTML"),
        )
...
async def signal_scan_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for key,snap in results:
            ...
            statuses=vip_statuses(recipients)
            for uid in recipients:
                is_user_vip = bool(statuses.get(int(uid), False))
                if not is_user_vip:
                    if not _reserve_free_signal_push(uid):
                        continue
                free_note="" if is_user_vip else "\n\n🔓 <i>نسخه رایگان: روزانه یک فرصت منتخب؛ پیگیری زنده کامل مخصوص VIP است.</i>"
                text=(
                ...
                async def on_success(uid=uid, is_user_vip=is_user_vip):
                    if not is_user_vip:
                        _mark_free_signal_push(uid)
                async def on_failure(uid=uid, is_user_vip=is_user_vip):
                    if not is_user_vip:
                        _release_free_signal_push(uid)
                try:
                    await notification_queue.put(
                        f"signal-push:{sid}:{uid}",
                        lambda uid=uid, text=text: context.bot.send_message(uid,text,parse_mode="HTML"),
                        on_success=on_success,
...
def run():
    ...
    logger.info("Notification queue configured | workers=%s maxsize=%s", NOTIFY_WORKERS, NOTIFY_QUEUE_MAXSIZE)
